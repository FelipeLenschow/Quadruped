...
class PolicyBridge(Node):
    def __init__(self, backend, checkpoint, robot_key, obs_dim=49, sim_type=None):
        ...
        print(f"[PolicyBridge] Backend: {backend.upper()} | Robot: {robot_key} | Policy: {checkpoint}")
    ...
    def _init_sim(self, sim_type):
        ...
        if sim_type:
            bridge_script = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", 
            ...
            self.sim_proc = subprocess.Popen(["/usr/bin/python3", bridge_script, f"--robot={self.robot_key}"],
                                              env=sim_env)
        else:
            self.sim_proc = None
    ...
    def _control_step_sim(self):
        if self.imu_data is None or self.joint_data is None: return
        
        # 1. Build Obs
        state = type('obj', (object,), {
            'imu': type('obj', (object,), self.imu_data),
            'motorState': [type('obj', (object,), {'q': q, 'dq': dq}) for q, dq in zip(self.joint_data['q'], self.joint_data['dq'])],
        ...
        if self._print_counter % 25 == 0:
            print(f"\r[PolicyBridge] Active cmd_vel: {self.cmd_vel} | vx={self.base_lin_vel[0]:.3f} | actions_mean={np.mean(self.last_actions):.3f}   ", end="", flush=True)
    ...
    def _control_step_real(self):
        ...
        obs = self.runner.build_obs(state_obj, self.cmd_vel, self.last_actions, self.desired_qpos, self.mj_to_isaac)
...
def main():
    ...
    if not os.path.isabs(ckpt):
        p_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "policies", ckpt))
        if os.path.exists(p_path): ckpt = p_path
