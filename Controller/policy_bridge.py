...
class PolicyController(Node):
    ...
    def __init__(self, checkpoint, robot_key, obs_dim=49):
        ...
        print(f"[PolicyController] Initialized for {robot_key} with policy {checkpoint}")
        ...
        self.mj_to_isaac = np.arange(12) # Topics are already standardized
        ...
        print("[PolicyController] Pure ROS controller started.")
    ...
    def control_loop(self):
        ...
        if self._print_counter % 2 == 0:
             print(f"\r[Controller] cmd: {self.cmd_vel[:3]} | vx: {self.base_lin_vel[0]:.3f} | act_mean: {np.mean(np.abs(actions)):.3f}   ", end="", flush=True)
