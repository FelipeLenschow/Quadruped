...
complete -o nospace -F _ut_rl_lab_python_argcomplete_wrapper "./simple.sh"


_ut_setup_conda_env() {

    # copied from isaaclab/_isaac_sim/setup_conda_env.sh
    # add source simple.sh to conda activate.d
    printf '%s\n' '#!/usr/bin/env bash' '' \
        '# for Isaac Lab' \
        ...
        '' \
        '# for simple' \
        'source '${UNITREE_RL_LAB_PATH}'/simple.sh' \
...
case "$1" in
    -i|--install)
        ...
        pip install -e ${UNITREE_RL_LAB_PATH}/source/simple/
        _ut_setup_conda_env
