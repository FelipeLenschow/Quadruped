...
- Download unitree robot description files
  ...
  *Method 1: Using USD Files*
  - Download unitree usd files from [unitree_model](https://huggingface.co/datasets/unitreerobotics/unitree_model/tree/main), keeping folder structure
    ...
    ```
  - Config `UNITREE_MODEL_DIR` in `source/simple/simple/assets/robots/unitree.py`.
  ...
  *Method 2: Using URDF Files [Recommended]* Only for Isaacsim >= 5.0
  -  Download unitree robot urdf files from [unitree_ros](https://github.com/unitreerobotics/unitree_ros)
      ...
      ```
  - Config `UNITREE_ROS_DIR` in `source/simple/simple/assets/robots/unitree.py`.
    ```bash
...
- Verify that the environments are correctly installed by:
  ...
  - Listing the available tasks:
    ...
    ```bash
    ./simple.sh -l # This is a faster version than isaaclab
    ```
  - Running a task:

    ```bash
    ./simple.sh -t --task Unitree-G1-29dof-Velocity # support for autocomplete task-name
    # same as
  ...
  - Inference with a trained agent:
    ...
    ```bash
    ./simple.sh -p --task Unitree-G1-29dof-Velocity # support for autocomplete task-name
    # same as
...
cd simple/deploy/robots/g1_29dof # or other robots
mkdir build && cd build
cmake .. && make
```
...
```bash
cd simple/deploy/robots/g1_29dof/build
./g1_ctrl
