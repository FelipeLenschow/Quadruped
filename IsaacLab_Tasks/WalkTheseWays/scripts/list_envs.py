...
All the environments are registered in the `walk_these_ways` extension. They start
with `Go2` in their name.
"""
...
def import_packages():
    sys.path.insert(0, f"{pathlib.Path(__file__).parent.parent}/source/walk_these_ways/walk_these_ways/tasks/")
...
def main():
    """Print all environments registered in `walk_these_ways` extension."""
    # print all the available environments
    table = PrettyTable(["S. No.", "Task Name", "Entry Point", "Config"])
    table.title = "Available Environments in Walk These Ways"
    ...
    for task_spec in gym.registry.values():
        if "Go2" in task_spec.id and "Isaac" not in task_spec.id:
