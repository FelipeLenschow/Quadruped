...
from walk_these_ways.assets.robots.go2 import GO2_CFG as ROBOT_CFG
from walk_these_ways.tasks.locomotion import mdp
...
# RobotEnvCfg (the base velocity config as shipped) plus: command-scaled tracking kernel, base_lin_vel in
...
class PolicyWithLinVelCfg(ObsGroup):
    """The base policy observation group with base_lin_vel prepended.
...
def _apply_lin_vel_obs(cfg) -> None:
    ...
    The base actor has no velocity input: it is told what speed to go and never told what speed
...
# Walk's speed-dependent swing-time term, replacing the base feet_air_time (threshold 0.5 s,
...
# base_lin_vel: the base policy group already carries more than Final7 did on every other
...
# and the motor zero-point calibration, which is per joint and the one hardware error a
...
# Gaussian sigma on the gyro, rad/s. The base config ships +-0.2 uniform (sigma 0.115), which is ~10x a
...
def _apply_actuator_delay(cfg) -> None:
    ...
    TorqueSpeedActuatorCfg already derives from DelayedPDActuatorCfg, so this is only its delay
    bounds. Replaced rather than mutated: the actuator cfgs are shared with the module-level
    GO2_CFG, and mutating them would leak into any other cfg built in this process.
    """
...
# The Go2 asset config gives every joint the hip motor's torque-speed curve, but the calf sits
# behind an extra 1.92:1 knee reduction: 45 N*m and 15.65 rad/s at the joint, not 23.4 and 30.
# Isaac Lab 3.0 fixed this in its own Go2 config; this asset never got it.
...
def _apply_calf_reduction(cfg) -> None:
    ...
    if not DEPLOY_CALF_REDUCTION:
        ...
        return
    from walk_these_ways.assets.robots.actuators import GO2_KNEE_RATIO
...
# ── WTW ────────────────────────────────────────────────────────────────────────────────────
# Deploy plus Walk These Ways: one policy conditioned on body height, step frequency, gait
# (trot / pace / bound / pronk), swing height, body pitch and stance width, with a gait clock in
# the observation. The actor sees the Deploy 48 first, then the 8 gait commands and 4 clock
# values, so a runner only appends 12 numbers. A zero velocity command means stand, not march.
FEET = ["FL_foot", "FR_foot", "RL_foot", "RR_foot"]


def _wtw_env_range(name, lo, hi):
    return (float(os.environ.get(f"WTW_{name}_LO", lo)), float(os.environ.get(f"WTW_{name}_HI", hi)))


# Measured on the default pose: base 0.277 m above the ground, feet 0.34 m apart and 0.45 m long.
WTW_NOMINAL_HEIGHT = float(os.environ.get("WTW_NOMINAL_HEIGHT", 0.28))
WTW_STANCE_LENGTH = float(os.environ.get("WTW_STANCE_LENGTH", 0.45))
...
WTW_CURRICULUM_INIT = (
    float(os.environ.get("WTW_CURRICULUM_INIT_X", 0.2)),
    float(os.environ.get("WTW_CURRICULUM_INIT_YAW", 0.2)),
)
# Past actor observations stacked into the input, as WTW's 30-step history. 1 turns it off.
WTW_HISTORY = int(os.environ.get("WTW_HISTORY", 30))
# Walk These Ways' reward set and weights, as its train.py sets them (the Go2 port uses the same).
# Positive weights are multiplied by exp(negative ones / 0.02) in PositiveRewardManager; the
# contact and height terms are negative values under positive weights, as in WTW. Its
# action_smoothness terms act on joint targets, so their weights here carry action_scale^2.
WTW_WEIGHTS = dict(
    track_lin_vel_xy=1.0,
    track_ang_vel_z=0.5,
    gait_contact_force=4.0,
    gait_contact_vel=4.0,
    gait_body_height=10.0,
    gait_swing_height=-30.0,
    gait_body_pitch=-5.0,
    gait_raibert=-10.0,
    base_linear_velocity=-0.02,
    base_angular_velocity=-0.001,
    joint_torques=-1e-5,
    joint_acc=-2.5e-7,
    joint_vel=-1e-4,
    action_rate=-0.01,
    action_smoothness_1=-0.1 * 0.25**2,
    action_smoothness_2=-0.1 * 0.25**2,
    feet_slip=-0.04,
    collision=-5.0,
)
WTW_WEIGHTS.update(
    {k[len("WTW_W_"):].lower(): float(v) for k, v in os.environ.items() if k.startswith("WTW_W_")}
)


def _apply_wtw_commands(cfg) -> None:
    # With the curriculum the gait term samples velocity too, on its own clock.
    cfg.commands.base_velocity.resampling_time_range = (1e9, 1e9) if WTW_CURRICULUM else WTW_VEL_RESAMPLE
    cfg.commands.gait = mdp.GaitCommandCfg(
        resampling_time_range=WTW_VEL_RESAMPLE,
        curriculum=WTW_CURRICULUM,
        curriculum_init=WTW_CURRICULUM_INIT,
        foot_names=tuple(FEET),
        ranges=mdp.GaitCommandCfg.Ranges(**WTW_RANGES),
...
def _apply_wtw_observations(cfg) -> None:
    cfg.observations.policy.gait_command = ObsTerm(func=mdp.gait_command, params={"command_name": "gait"})
    cfg.observations.policy.gait_clock = ObsTerm(func=mdp.gait_clock, params={"command_name": "gait"})
    cfg.observations.critic.gait_command = ObsTerm(func=mdp.gait_command, params={"command_name": "gait"})
    cfg.observations.critic.gait_clock = ObsTerm(func=mdp.gait_clock, params={"command_name": "gait"})
    cfg.observations.critic.gait_desired_contact = ObsTerm(
        func=mdp.gait_desired_contact, params={"command_name": "gait"}
    )
    # Flattened term by term: each term's WTW_HISTORY steps, oldest first, then the next term.
    cfg.observations.policy.history_length = WTW_HISTORY if WTW_HISTORY > 1 else None
    print(f"[WTW] actor history {WTW_HISTORY} steps")


def _apply_wtw_rewards(cfg) -> None:
    r = cfg.rewards
    for name in ("dof_pos_limits", "energy", "flat_orientation_l2", "joint_pos", "hip_dev", "feet_air_time",
                 "feet_air_time_dyn", "foot_height", "air_time_variance", "feet_slide", "undesired_contacts"):
        setattr(r, name, None)

    r.track_lin_vel_xy = RewTerm(func=mdp.track_lin_vel_xy_exp, weight=1.0,
                                 params={"command_name": "base_velocity", "std": math.sqrt(0.25)})
    r.track_ang_vel_z = RewTerm(func=mdp.track_ang_vel_z_exp, weight=1.0,
                                params={"command_name": "base_velocity", "std": math.sqrt(0.25)})
    feet_asset = SceneEntityCfg("robot", body_names=FEET, preserve_order=True)
    feet_sensor = SceneEntityCfg("contact_forces", body_names=FEET, preserve_order=True)
    r.gait_contact_force = RewTerm(func=mdp.gait_contact_force, weight=1.0, params={"sensor_cfg": feet_sensor})
    r.gait_contact_vel = RewTerm(func=mdp.gait_contact_vel, weight=1.0, params={"asset_cfg": feet_asset})
    r.gait_body_height = RewTerm(
        func=mdp.gait_body_height,
        weight=1.0,
        params={"nominal_height": WTW_NOMINAL_HEIGHT, "sensor_cfg": SceneEntityCfg("height_scanner")},
    )
    r.gait_swing_height = RewTerm(func=mdp.gait_swing_height, weight=1.0, params={"asset_cfg": feet_asset})
    r.gait_body_pitch = RewTerm(func=mdp.gait_body_pitch, weight=1.0)
    r.gait_raibert = RewTerm(func=mdp.gait_raibert, weight=1.0,
                             params={"asset_cfg": feet_asset, "stance_length": WTW_STANCE_LENGTH})
    r.action_smoothness_1 = RewTerm(func=mdp.action_rate_l2, weight=1.0)
    r.action_smoothness_2 = RewTerm(func=mdp.action_smoothness_2, weight=1.0)
    r.feet_slip = RewTerm(func=mdp.feet_slip, weight=1.0,
                          params={"asset_cfg": feet_asset, "sensor_cfg": feet_sensor})
    r.collision = RewTerm(
        func=mdp.undesired_contacts,
        weight=1.0,
        params={"threshold": 0.1, "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[".*_thigh", ".*_calf"])},
    )
    for name, weight in WTW_WEIGHTS.items():
        getattr(r, name).weight = weight
    print(f"[WTW] nominal height {WTW_NOMINAL_HEIGHT} m, stance length {WTW_STANCE_LENGTH} m, weights {WTW_WEIGHTS}")


@configclass
class RobotWTWEnvCfg(RobotSigmaVelFootRoughDeployEnvCfg):
    positive_reward_sigma: float = float(os.environ.get("WTW_POSITIVE_SIGMA", 0.02))

    def __post_init__(self):
        super().__post_init__()
        if self.scene.height_scanner is None:
            raise ValueError("WTW needs the height scanner for body height; unset PAPER_NO_HEIGHT_SCANNER.")
        # The gait terms hold slow tracking under the level curriculum's bar for good (measured:
        # still at +-0.1 after 2400 iterations), so WTW starts at the full command range.
        if self.curriculum.lin_vel_cmd_levels is not None and os.environ.get("WTW_KEEP_CURRICULUM", "0") != "1":
            _drop_level_curriculum(self)
        _apply_wtw_commands(self)
        _apply_wtw_observations(self)
        _apply_wtw_rewards(self)


@configclass
class RobotWTWPlayEnvCfg(RobotWTWEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        _apply_play_overrides(self)
        self.commands.gait.curriculum = False
        self.commands.base_velocity.resampling_time_range = WTW_VEL_RESAMPLE
