"""
Eval Sweep teleop - the quadruped_drivers/eval_mujoco.py speed protocol, paced by hand on the real robot.
...
# Same table as MujocoEvaluator._evaluation_loop in quadruped_drivers/eval_mujoco.py, copied rather than
...
# F710 in X (XInput) mode, see quadruped_bringup/config/joy_f710.config.yaml.
...
def load_free_drive_cfg():
    ...
    fixed here too."""
    path = paths.config("joy_f710.config.yaml")
    try:
        with open(path) as f:
...
def main():
    ...
    # Taken from rclpy for the same reason as in quadruped_operator/console.py: its own SIGINT handler tears
