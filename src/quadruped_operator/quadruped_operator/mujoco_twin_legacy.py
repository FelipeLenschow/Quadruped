import os
import sys
import time
import threading
import numpy as np
import argparse

import rclpy
from rclpy.node import Node
from sensor_msgs.msg import Imu, JointState
from nav_msgs.msg import Odometry

import mujoco
import mujoco.viewer


from quadruped_core.telemetry.estimator import rot_from_quat
from quadruped_core import paths
from rclpy.utilities import remove_ros_args

class MujocoTwin(Node):
    """
    Passive MuJoCo Digital Twin.
    Listens to ROS 2 topics and updates the MuJoCo visualization without running physics.
    Integrates body velocity from /odom to estimate global position.
    """
    def __init__(self, robot_type="go2"):
        super().__init__("mujoco_twin_node")
        self.robot_type = robot_type

        # 1. Load MuJoCo Model
        mjcf_path = paths.description("mujoco_menagerie", "unitree_go2", "scene.xml")

        self.get_logger().info(f"Loading MuJoCo Twin Model from {mjcf_path}")
        self.model = mujoco.MjModel.from_xml_path(mjcf_path)
        self.data = mujoco.MjData(self.model)

        # Disable Gravity and Collisions to prevent physics interference 
        # (Though we won't call mj_step, it's safe to disable them)
        self.model.opt.gravity[:] = 0.0
        self.model.geom_conaffinity[:] = 0
        self.model.geom_contype[:] = 0

        # Resolve joint indices
        self.isaac_names = [
            "FL_hip_joint", "FR_hip_joint", "RL_hip_joint", "RR_hip_joint",
            "FL_thigh_joint", "FR_thigh_joint", "RL_thigh_joint", "RR_thigh_joint",
            "FL_calf_joint", "FR_calf_joint", "RL_calf_joint", "RR_calf_joint",
        ]
        self.qpos_addr = np.zeros(12, dtype=int)
        for i, name in enumerate(self.isaac_names):
            j_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_JOINT, name)
            if j_id != -1:
                self.qpos_addr[i] = self.model.jnt_qposadr[j_id]

        # 2. State Variables
        # Start floating at z=0.35 with identity quaternion
        self.base_pos = np.array([0.0, 0.0, 0.35], dtype=np.float64)
        self.base_quat = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float64) # [w, x, y, z]
        self.joint_pos = np.zeros(12, dtype=np.float64)
        self.base_lin_vel_body = np.zeros(3, dtype=np.float64)

        self.last_time = time.time()
        self.lock = threading.Lock()

        # 3. ROS Subscriptions
        self.create_subscription(JointState, "/sensors/joint_states", self.joint_cb, 10)
        self.create_subscription(Imu, "/sensors/imu", self.imu_cb, 10)
        self.create_subscription(Odometry, "/odom", self.odom_cb, 10)

        # 4. Start Viewer Thread
        self.viewer_thread = threading.Thread(target=self._viewer_loop, daemon=True)
        self.viewer_thread.start()

        self.get_logger().info("MuJoCo Digital Twin is running. Waiting for telemetry...")

    def joint_cb(self, msg: JointState):
        with self.lock:
            # Match names to indices
            for i, name in enumerate(msg.name):
                try:
                    idx = self.isaac_names.index(name)
                    self.joint_pos[idx] = msg.position[i]
                except ValueError:
                    pass

    def imu_cb(self, msg: Imu):
        with self.lock:
            q = msg.orientation
            # ROS is usually [x, y, z, w], we want [w, x, y, z] for MuJoCo
            # Wait, our telemetry publishes: w=float(q[0]), x=float(q[1]), y=float(q[2]), z=float(q[3])
            # So the msg has w, x, y, z explicitly defined.
            self.base_quat = np.array([q.w, q.x, q.y, q.z])

    def odom_cb(self, msg: Odometry):
        with self.lock:
            # Telemetry manager publishes body-frame velocity here
            v = msg.twist.twist.linear
            self.base_lin_vel_body = np.array([v.x, v.y, v.z])

    def _viewer_loop(self):
        with mujoco.viewer.launch_passive(self.model, self.data) as viewer:
            # Optional: Track the robot
            viewer.cam.type = mujoco.mjtCamera.mjCAMERA_TRACKING
            track_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "trunk")
            if track_id == -1:
                track_id = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY, "base")
            if track_id != -1:
                viewer.cam.trackbodyid = track_id

            while rclpy.ok() and viewer.is_running():
                current_time = time.time()
                dt = current_time - self.last_time
                self.last_time = current_time

                with self.lock:
                    # 1. Integrate position (rotate body velocity to world frame)
                    R = rot_from_quat(self.base_quat)
                    v_world = R @ self.base_lin_vel_body
                    
                    # Update X and Y only. Keep Z fixed or bounded to prevent falling/flying due to drift.
                    self.base_pos[0] += v_world[0] * dt
                    self.base_pos[1] += v_world[1] * dt
                    
                    # Update MuJoCo Data
                    # Base pos [x, y, z]
                    self.data.qpos[0:3] = self.base_pos
                    # Base quat [w, x, y, z]
                    self.data.qpos[3:7] = self.base_quat
                    # Joints
                    for i, addr in enumerate(self.qpos_addr):
                        if addr > 0:
                            self.data.qpos[addr] = self.joint_pos[i]

                # 2. Kinematic Forward (NO mj_step!)
                mujoco.mj_forward(self.model, self.data)
                
                # 3. Update Viewer
                viewer.sync()
                
                # Render at ~60 Hz
                time.sleep(1.0 / 60.0)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--robot", type=str, default="go2")
    args = parser.parse_args(remove_ros_args()[1:])

    rclpy.init()
    node = MujocoTwin(robot_type=args.robot)
    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass
    node.destroy_node()
    rclpy.shutdown()

if __name__ == "__main__":
    main()
