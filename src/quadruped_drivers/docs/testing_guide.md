...
3. **Verify Config**: Check `src/quadruped_bringup/config/config.yaml` to ensure `saturation_limit` is set to `0.9` (default for safety).
...
```bash
ros2 launch quadruped_bringup real.launch.py robot:=go2
```
