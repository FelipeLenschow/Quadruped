...
# so on its own timer it does not overrun (scripts/profile_control_loop.py measures it).
#
...
class RealDriver(Node):
    def __init__(self, robot="go2", internal_policy=None, obs_dim=45, interface=None):
        ...
        # viewer runs on the operator's laptop, off a DIFFERENT quadruped_bringup/config/config.yaml
...
def main():
    ...
    with open(paths.config("config.yaml"), 'r') as f:
        cfg_data = yaml.safe_load(f)
