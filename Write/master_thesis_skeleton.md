...
* **3.3 Hardware-Agnostic Core Components**
  ...
  * **`Controller/` Package** (left and center of diagram):
    * **`PolicyManager` (`policy_manager.py`):** The policy registry and coordinator.
      * Loads multiple JIT or standard PyTorch models (e.g. main and safety backup).
      * Performs observation scaling and coordinate centering internally using `action_scale` and `desired_qpos`.
      * Lazily steps and scales only the active policy to save computational resources.
      * Exposes proposed absolute joint targets in radians.
    * **`CommandSafetyProcessor` (`command_safety_processor.py`):** The policy-agnostic safety gate and arbitrator.
      * Evaluates tilt, pitch, and joint ROM safety boundaries internally at the policy frequency (50Hz).
      * Subscribes to separate Supervisor heartbeat topics (`/safety/*`) for safety thresholds, enabling hot-swapping parameters from outside.
      * Automatically clips targets to soft ranges of motion to prevent structural collisions.
      * Runs an asynchronous enter-to-reset loop to restore blocked main policies.
    * **`Distributor` (`distributor.py`):** The clean exit-point for joint commands, wrapping standard ROS 2 `/commands/joint_commands` state publishing and driver callbacks.
...
* **3.5 Operator Monitoring Layer** (bottom of diagram)
  * **`Operator/` Package:**
    * **`SupervisorNode` (`supervisor.py`):** Decoupled safety configuration heartbeat broadcaster.
      ...
      * Renders all metrics periodically for real-time operator diagnostics.
...
* **6.2 Safety Architecture**
  ...
  * The Supervisor-CommandSafetyProcessor relationship as a model for safe RL deployment.
